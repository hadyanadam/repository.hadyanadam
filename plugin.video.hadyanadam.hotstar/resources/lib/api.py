...
class HotstarAPI:
    ...
    def getMenu(self):
        ...
        resp = self.get(
            url, headers={"x-country-code": "id", "x-platform-code": "ANDROID_TV"})
    ...
    def doLogin(self):
        url = url_constructor(
            "/id/aadhar/v2/firetv/id/users/logincode/")
    ...
    def _refreshToken(self):
        try:
            with PersistentDict("userdata.pickle") as db:
                ...
                if oldToken:
                    resp = self.session.get(url_constructor("/id/aadhar/v2/firetv/id/users/refresh-token"),
